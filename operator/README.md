...
### Push the Operator Image to a Registry

```bash
bazel run //operator:image-push [-- --repository=docker.io/example/vimana-operator]
```

If `--repository` is not specified, `localhost:5000/vimana-operator` is used by default.
...
In particular, `make generate` and `make manifests` were combined
and converted into a single executable rule:

```bash
bazel run //operator:generate
```

**This mutates the source tree in-place**,
just like those Make commands would in a normal Operator SDK project.
Just run that rule after making any changes to the operator.
